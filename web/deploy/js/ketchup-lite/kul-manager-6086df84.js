    ...
    logs = {
        ...
        new: async (comp, message, category = 'informational') => {
            ...
            if (this.#LOGS.length > this.#LOG_LIMIT) {
                if (this.isEnabled()) {
                    console.warn(dom$4.ketchupLite.dates.format(log.date, 'LLL:ms') +
                        ' kul-debug => ' +
                        'Too many logs (> ' +
                        this.#LOG_LIMIT +
            ...
            switch (category) {
                case 'error':
                    console.error(dom$4.ketchupLite.dates.format(log.date, 'LLL:ms') +
                        log.id +
        ...
        print: () => {
            ...
            for (let index = 0; index < this.#LOGS.length; index++) {
                ...
                const printEntry = {
                    class: log.class,
                    date: dom$4.ketchupLite.dates.format(log.date, 'LLL:ms'),
...
class KulDynamicPosition {
        ...
        const mutObserver = new MutationObserver(function (mutations) {
            ...
            if (target.classList.contains(KUL_DROPDOWN_CLASS_VISIBLE)) {
                requestAnimationFrame(function () {
                    dom$3.ketchupLite.dynamicPosition.run(el);
                });
            }
        });
    ...
    run(el) {
        if (!el.isConnected) {
            dom$3.ketchupLite.dynamicPosition.managedElements.delete(el);
...
class KulLanguage {
    ...
    translate(key, language) {
        ...
        function invalidKey(key) {
            dom$2.ketchupLite.debug.logs.new(this, 'Invalid translation for key (' + key + ')!', 'warning');
            return key;
        }
    }
    set(language) {
        if (language && typeof language === 'string') {
            ...
            dom$2.ketchupLite.debug.logs.new(this, "Couldn't set language, invalid string received (" +
                language +
                ')!', 'warning');
            return;
        }
        ...
        if (this.list[dLanguage]) {
            if (dVariant && !this.list[dLanguage].variants[dVariant]) {
                dom$2.ketchupLite.debug.logs.new(this, 'Variant not found (' + dVariant + ')!', 'warning');
                return;
            }
        }
        else {
            dom$2.ketchupLite.debug.logs.new(this, 'Language not found (' + dLanguage + ')!', 'warning');
            return;
        }
        this.name = language;
        this.managedComponents.forEach(function (comp) {
...
//# sourceMappingURL=kul-manager-6086df84.js.map