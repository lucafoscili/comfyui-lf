...
class KulDebug {
    ...
    logs = {
        ...
        new: async (comp, message, category = 'informational') => {
            ...
            if (this.#LOGS.length > this.#LOG_LIMIT) {
                if (this.#IS_ENABLED) {
                    console.warn(this.#DOM.ketchupLite.dates.format(log.date, 'LLL:ms') +
                        ' kul-debug => ' +
                        'Too many logs (> ' +
                        this.#LOG_LIMIT +
            ...
            switch (category) {
                case 'error':
                    console.error(this.#DOM.ketchupLite.dates.format(log.date, 'LLL:ms') +
                        log.id +
    ...
    register(comp) {
        if (comp.rootElement.tagName.toLowerCase() === 'kul-code') {
            ...
            this.#MANAGED_COMPONENTS.togglees.add(comp);
        }
    }
    ...
    unregister(comp) {
        if (comp.rootElement.tagName.toLowerCase() === 'kul-code') {
            ...
            this.#MANAGED_COMPONENTS.togglees.delete(comp);
        }
    }
...
class KulLanguage {
    ...
    set(language) {
        if (language && typeof language === 'string') {
            ...
            this.dom.ketchupLite.debug.logs.new(this, "Couldn't set language, invalid string received (" +
                language +
                ')!', 'warning');
            return;
        }
...
class KulScrollOnHover {
                ...
                if (direction) {
                    ...
                    this.#timeout = setTimeout(() => {
                        ...
                        this.#rAF = requestAnimationFrame(function () {
                            this.#DOM.ketchupLite.scrollOnHover.run(el, maxScrollLeft, percRight, percLeft, direction);
                        });
                    }, this.delay);
                }
            }
            ...
            if (trueHeight !== 0 && !el.scrollOnHover.active) {
                ...
                if (direction) {
                    this.#timeout = setTimeout(() => {
                        el.scrollOnHover.active = true;
                        this.#rAF = requestAnimationFrame(function () {
                            this.#DOM.ketchupLite.scrollOnHover.run(el, maxScrollTop, percBottom, percTop, direction);
                        });
                    }, this.delay);
                }
            }
    ...
    run(el, maxScrollLeft, percForward, percBack, direction) {
        ...
        this.#rAF = requestAnimationFrame(function () {
            this.#DOM.ketchupLite.scrollOnHover.run(el, maxScrollLeft, percForward, percBack, direction);
        });
    }
    updateChildren(el) {
...
class KulTheme {
    ...
    set = (name, list) => {
        ...
        this.#DOM.ketchupLite.debug.logs.new(this, 'Setting theme to: ' + this.name + '.');
        const theme = this.list?.[this.name];
        if (!theme) {
            this.#DOM.ketchupLite.debug.logs.new(this, 'Invalid theme name, falling back to default ("silver").');
            this.name = 'silver';
        }
        ...
        try {
            this.styleTag.innerText =
                ':root[kul-theme="' +
                    ...
                    '}';
            this.#customStyle();
            this.#DOM.ketchupLite.debug.logs.new(this, 'Theme ' + this.#DOM.getAttribute('kul-theme') + ' refreshed.');
            document.dispatchEvent(new CustomEvent('kul-theme-refresh'));
        }
        catch (error) {
            this.#DOM.ketchupLite.debug.logs.new(this, 'Theme not refreshed.', 'warning');
        }
    };
    ripple = {
        setup: (el) => {
    ...
    randomTheme = () => {
        ...
        if (themes.length > 0) {
            ...
            this.#DOM.ketchupLite.debug.logs.new(this, "Couldn't set a random theme: no themes available!", 'warning');
        }
    };
    colorCheck = (color) => {
        if (color === 'transparent') {
            color = this.cssVars['--kul-background-color'];
            this.#DOM.ketchupLite.debug.logs.new(this, 'Received TRANSPARENT color, converted to ' +
                color +
                ' (theme background).');
        }
        ...
        if (!isHex && !isHsl && !isRgb) {
            ...
            this.#DOM.ketchupLite.debug.logs.new(this, 'Received CODE NAME color ' +
                oldColor +
                ', converted to ' +
                color +
                '.');
        ...
        if (isHex || isHsl) {
            ...
            try {
                ...
                this.#DOM.ketchupLite.debug.logs.new(this, 'Received HEX color ' +
                    oldColor +
                    ', converted to ' +
                    color +
                    '.');
                ...
                this.#DOM.ketchupLite.debug.logs.new(this, 'Invalid color: ' + color + '.');
            }
        }
        let rgbValues = '';
        ...
        try {
            ...
            this.#DOM.ketchupLite.debug.logs.new(this, 'Color not converted to rgb values: ' + color + '.');
        }
        if (!hexColor) {
            try {
                ...
                this.#DOM.ketchupLite.debug.logs.new(this, 'Color not converted to hex value: ' + color + '.');
            }
        }
    ...
    codeToHex(color) {
        ...
        if (colorCodes[color.toLowerCase()]) {
            ...
            this.#DOM.ketchupLite.debug.logs.new(this, 'Could not decode color ' + color + '!');
            return color;
        }
    }
}
