...
var _LFManager_instances,
  _LFManager_CSS_EMBEDDED,
  _LFManager_DEBUG,
  _LFManager_DOM,
  _LFManager_EXT_PREFIX,
  _LFManager_MANAGERS,
  _LFManager_NODES_DICT,
  _LFManager_embedCss,
  _LFManager_registerControlPanel;
...
export class LFManager {
  constructor() {
    ...
    const managerCb = () => {
      ...
      this.log(
        'KulManager ready',
        { kulManager: __classPrivateFieldGet(this, _LFManager_MANAGERS, 'f').ketchupLite },
        'success',
      );
    ...
    for (const key in __classPrivateFieldGet(this, _LFManager_NODES_DICT, 'f')) {
      if (
        Object.prototype.hasOwnProperty.call(
          __classPrivateFieldGet(this, _LFManager_NODES_DICT, 'f'),
          key,
        )
      ) {
        ...
        switch (key) {
          case 'controlPanel':
            __classPrivateFieldGet(this, _LFManager_instances, 'm', _LFManager_embedCss).call(
              this,
              key,
            );
            __classPrivateFieldGet(
              this,
              _LFManager_instances,
              'm',
              _LFManager_registerControlPanel,
            ).call(this);
            break;
  ...
  toggleDebug(value) {
    if (value === false || value === true) {
      ...
      __classPrivateFieldSet(
        this,
        _LFManager_DEBUG,
        !__classPrivateFieldGet(this, _LFManager_DEBUG, 'f'),
        'f',
      );
    }
    this.log(
      `Debug active: '${__classPrivateFieldGet(this, _LFManager_DEBUG, 'f')}'`,
      {},
      'warning',
    );
    return __classPrivateFieldGet(this, _LFManager_DEBUG, 'f');
...
(_LFManager_CSS_EMBEDDED = new WeakMap()),
  (_LFManager_DEBUG = new WeakMap()),
  (_LFManager_DOM = new WeakMap()),
  (_LFManager_EXT_PREFIX = new WeakMap()),
  (_LFManager_MANAGERS = new WeakMap()),
  (_LFManager_NODES_DICT = new WeakMap()),
  (_LFManager_instances = new WeakSet()),
  (_LFManager_embedCss = function _LFManager_embedCss(filename) {
      ...
      beforeRegisterNodeDef: async (nodeType) => {
        if (nodeType.comfyClass === this.CONTROL_PANEL.nodeName) {
          ...
          nodeType.prototype.onNodeCreated = function () {
            ...
            const node = this;
            __classPrivateFieldGet(self, _LFManager_MANAGERS, 'f').widgets.create.controlPanel(
              node,
            );
            return r;
          };
