...
export const imageEditorFactory: ImageEditorWidgetFactory = {
  ...
  render: (node) => {
    ...
    tree.addEventListener(
      'kul-tree-event',
      treeEventHandler.bind(treeEventHandler, image, settings),
    );
    ...
    button.addEventListener(
      'kul-button-event',
      buttonEventHandler.bind(buttonEventHandler, textfield, options.refresh),
    );
    ...
    masonry.addEventListener(
      'kul-masonry-event',
      masonryEventHandler.bind(masonryEventHandler, grid, image),
    );
...
const buttonEventHandler = async (
  ...
  refreshCb: (value: string) => Promise<void>,
...
const masonryEventHandler = async (
  grid: HTMLDivElement,
...
const prepSettings = (settings: HTMLDivElement, node: KulDataNode, image: HTMLKulImageElement) => {
  settings.innerHTML = '';
