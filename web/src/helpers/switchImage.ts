...
const eventName: EventNames = 'lf-switchimage';

const eventCb = (event: CustomEvent<SwitchImagePayload>) => {
...
const updateCb = (node: NodeType) => {
  const props = node.lfProps as SwitchImageProps;
  if (window.lfManager.getDebug()) {
...
export const SwitchImageAdapter: () => SwitchImageDictionaryEntry = () => {
  return {
    eventCb,
    eventName,
    updateCb,
