import { ImageClassifierPayload, EventName } from '../types/events';
import { LogSeverity } from '../types/manager';
import { NodeName, type Extension } from '../types/nodes';
import { CustomWidgetName, type BaseWidgetCallback, type CodeWidgetSetter } from '../types/widgets';
import { getApiRoutes, getCustomWidget, getLFManager } from '../utils/common';

const NAME = NodeName.imageClassifier;

export const imageClassifierFactory = {
  eventHandler: (
    event: CustomEvent<ImageClassifierPayload>,
    addW: BaseWidgetCallback<CustomWidgetName.code>,
  ) => {
    const name = EventName.imageClassifier;
